...
function Navbar() {
  ...
  return (
    <div className='w-full h-14 sticky top-0 z-50 flex items-center justify-between bg-white dark:bg-dark-900 border-b border-gray-200 dark:border-gray-800 px-6'>
      <div className='flex items-center'>
        <img src={iconImage} alt='App Icon' className='w-8 h-8 mr-1' />
        <h2 className='text-lg font-bold dark:text-gray-100'>{APP_NAME}</h2>
      </div>
      <div className='flex items-center space-x-3'>
        ...
        <button
          onClick={toggleTheme}
          className={`relative flex items-center w-10 h-5 rounded-full border transition-colors duration-300 border hover:border-red-400 mt-1 cursor-pointer ${theme === 'dark' ? 'bg-gray-800 border-red-400' : 'bg-gray-200 border-gray-400'
            }`}
        >
          <span
            className={`absolute top-0.1 left-0.5 flex items-center justify-center w-4 h-4 rounded-full transition-all duration-300 ${theme === 'dark' ? 'translate-x-5 bg-black text-gray-200' : 'translate-x-0 bg-white text-yellow-500'
              }`}
          >
