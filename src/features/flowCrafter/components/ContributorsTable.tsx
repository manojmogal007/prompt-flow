...
const getStatus = (status: string) => {
  switch (status) {
    ...
    default:
      return 'bg-gray-100 text-gray-800 dark:bg-dark-700 dark:text-dark-300';
  }
};
    ...
    <div className='w-full bg-white dark:bg-dark-800 rounded-lg shadow-sm border border-gray-200 dark:border-dark-700'>
          ...
          <tbody className='bg-white dark:bg-dark-800 divide-y divide-gray-200 dark:divide-dark-700'>
            {isLoading ? (
              ...
              contributors.map((contributor) => {
                return (
                  <tr key={contributor._id} className={`hover:bg-gray-50 dark:hover:bg-dark-700/50 transition-colors`}>
                    {/* Workflow Name & Info */}
                          ...
                          <div className='w-[100px] flex items-center space-x-2'>
                            ...
                            <Tooltip text='Save changes' width={100}>
                              ...
                              <button
                                ...
                                className='text-gray-400 dark:text-dark-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors'
                              >
                                <Save className='w-4 h-4 text-gray-400 dark:text-dark-400 hover:text-blue-600' />
                              </button>
                            </Tooltip>
                          </div>
                        ) : (
      ...
      {!isLoading && contributors.length === 0 && (
        <div className='text-center py-12'>
          <Users className='mx-auto h-12 w-12 text-gray-400 dark:text-dark-500' />
          <h3 className='mt-2 text-sm font-medium text-gray-900 dark:text-dark-100'>No contributors found</h3>
          <p className='mt-1 text-sm text-gray-500 dark:text-dark-400'>Start collaborating by adding your first contributor</p>
        </div>
      )}
    </div>
  );
