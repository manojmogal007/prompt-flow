...
export const UserCard: React.FC<UserCardProps> = ({ user, onClick }) => {
    ...
    const getRoleStyles = () => {
        if (isSuperAdmin) {
            return {
                container: 'bg-gradient-to-br from-amber-50/80 to-red-50/50 dark:from-amber-900/10 dark:to-red-900/10 border-amber-200 dark:border-amber-500/30 shadow-amber-100 dark:shadow-none',
        ...
        if (isAdmin) {
            return {
                container: 'bg-gradient-to-br from-indigo-50/80 to-violet-50/50 dark:from-indigo-900/10 dark:to-violet-900/10 border-indigo-200 dark:border-indigo-500/30 shadow-indigo-100 dark:shadow-none',
      ...
      `}
            ...
            `}
                        ...
                        <div>
                            ...
                            {/* Role Badge inline */}
                            <span className={`text-[10px] font-bold uppercase tracking-wider mt-0.5 inline-block ${isSuperAdmin ? 'text-amber-600 dark:text-amber-400' : isAdmin ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-400'
                                }`}>
          ...
          `}
                    ...
                    <div>
                        ...
                        <div className='h-1.5 w-full bg-gray-100 dark:bg-dark-700 rounded-full overflow-hidden'>
                            <div
                                ...
                                style={{ width: `${Math.min(((user?.workflowCreationUsage || 0) / Math.max(user?.workflowCreationLimit || 1, 1)) * 100, 100)}%` }}
                            />
                        </div>
                    </div>
                ...
                <div className='pt-3 border-t border-gray-100 dark:border-dark-700/50 flex items-center justify-between text-xs text-gray-400'>
                    ...
                    <div className={`font-medium uppercase tracking-wider text-[10px] ${isUnlimited ? 'text-amber-600 dark:text-amber-400 font-bold' : 'text-gray-500 dark:text-gray-400'
                        }`}>
